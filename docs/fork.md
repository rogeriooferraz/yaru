...
Create or refresh the local build environment, then validate:
...
After a rebase, recreate the ignored build tree so files generated by the old
upstream build graph cannot affect validation:

```bash
scripts/setup-venv --wipe
```

When no rebase occurred, refresh the existing configuration incrementally:

```bash
scripts/setup-venv
```

Then review and build the customization:
